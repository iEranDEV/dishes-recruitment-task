...
export default function Input({ title, name, type, error, validators, register }: InputProps) {
    ...
    return (
        <div className="flex flex-col gap-1">
            <label htmlFor={name} className="text-gray-500 font-semibold w-full">{title} <span className="text-gray-400">*</span></label>
            <input 
                type={type}
                id={name}
                step={type === 'time' ? 1 : 'any'}
                className={`bg-gray-50 border ${error ? 'border-red-500' : 'border-gray-300'} px-2 py-1 rounded-lg w-full`}
